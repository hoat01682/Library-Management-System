 ...
 * @author Duc3m
 */
public class GetPublisherDialog extends javax.swing.JDialog {

    PublisherBUS publisherBUS = new PublisherBUS();
    ArrayList<PublisherDTO> publisherList;
    public boolean choosen = false;
    
    public GetPublisherDialog(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        customInit();
    }
    ...
    public static PublisherDTO getPublisher() {
        GetPublisherDialog gpD = new GetPublisherDialog(null, true);
        gpD.setVisible(true);
        try {
            if (gpD.choosen == false) {
                return null;
            }
            int publisher_id = gpD.getSelectedId();
            return PublisherBUS.getInstance().getById(publisher_id);
        } catch (Exception ex) {
    ...
    private void initComponents() {
        ...
        jLabel1.setText("CHỌN NHÀ XUẤT BẢN");
